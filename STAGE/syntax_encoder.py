...
class SyntaxAwareGCN(nn.Module):
    ...
    def __init__(self, dep_dim, in_features, out_features, pos_dim=None, bias=True):
        ...
        self.dep_attn = nn.Linear(dep_dim + pos_dim + out_features, out_features)
        self.dep_fc = nn.Linear(dep_dim, out_features)
        self.pos_fc = nn.Linear(pos_dim, out_features)
        
        self.shrinker = nn.Linear(in_features, out_features)
        self.shrinker_2 = nn.Linear(in_features, out_features)
...
class SGCN(nn.Module):
    def __init__(self, opt):
        ...
        self.opt = opt
        self.model = nn.ModuleList([SyntaxAwareGCN(opt.dep_dim, opt.bert_feature_dim,
                                                  opt.bert_feature_dim, opt.pos_dim)
...
class SyMuxEncoder(nn.Module):
    def __init__(self, opt):
        ...
        self.opt = opt
        self.bert = BertModel.from_pretrained(opt.pretrained_model)
        ...
        self.fc = nn.Linear(opt.bert_feature_dim + opt.bert_feature_dim + opt.dep_dim, opt.bert_feature_dim)
    ...
    def forward(self, input_ids, input_masks, simple_graph, graph, pos=None, output_attention=False):
        ...
        sequence_output = self.bert(input_ids, input_masks).last_hidden_state
