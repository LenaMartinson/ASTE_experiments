...
class ASTE_End2End_Dataset(Dataset):
    def __init__(self, file_name, 
        ...
        self.add_pos_mode = add_pos_mode
        self.add_simple_mode = add_simple_mode
    ...
    def preprocess(self, data, vocab, version):
        ...
        for d in tqdm(data, 'Loading data...'):
            ...
            tok = tok[:length]
            adj_matrix, adj_matrix_simple, pos_tags = make_adj_matrix(
                bert_token, 
                self.tokenizer, 
                len(bert_token), 
                SEP_id, 
                add_pos_mode=self.add_pos_mode,
                add_simple_mode=self.add_simple_mode
            )
            tok = [token_vocab.stoi.get(t, token_vocab.unk_index) for t in tok]
                        
            temp = {
                'adj_matrix': adj_matrix,
                'adj_matrix_simple':adj_matrix_simple,
                'pos_tags': pos_tags,
                'token': tok,
                'token_length': length,
...
def ASTE_collate_fn(batch):
    ...
    for i in range(batch_size):
        max_batch_len = max(max_batch_len, len(batch[i]['bert_token']))
    
    adj_matrix = torch.cat([
        torch.tensor(np.pad(batch[i]['adj_matrix'], [(0, max_batch_len - batch[i]['adj_matrix'].shape[0]), 
                                         (0, max_batch_len - batch[i]['adj_matrix'].shape[0])])).unsqueeze(0) for i in range(batch_size)], axis=0)
    adj_matrix_simple = torch.cat([
        torch.tensor(np.pad(batch[i]['adj_matrix_simple'], [(0, max_batch_len - batch[i]['adj_matrix_simple'].shape[0]), 
                                         (0, max_batch_len - batch[i]['adj_matrix_simple'].shape[0])])).unsqueeze(0) for i in range(batch_size)], axis=0)
    
    pos_tags = torch.cat(
        [torch.tensor(np.pad(batch[i]['pos_tags'], (0, max_batch_len - len(batch[i]['pos_tags']))))
         .unsqueeze(0) for i in range(batch_size)], axis=0)
